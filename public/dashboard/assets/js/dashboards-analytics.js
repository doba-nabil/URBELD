...
(function () {
    ...
    // Average Daily Sales (Monthly Requests Chart)
    // --------------------------------------------------------------------
    const averageDailySalesEl = document.querySelector('#averageDailySales');
    let averageDailySalesConfig = {
        chart: {
            height: 105,
            type: 'area',
            toolbar: {
    ...
    // Get weekly data from page if available
    if (typeof averageDailySalesEl !== undefined && averageDailySalesEl !== null) {
        // Try to get data from window object or use default
        if (typeof window.weeklyRequestsData !== 'undefined' && Array.isArray(window.weeklyRequestsData)) {
            averageDailySalesConfig.series[0].data = window.weeklyRequestsData.map(item => item.count);
        } else {
            // Default data
            averageDailySalesConfig.series[0].data = [0, 0, 0, 0, 0, 0, 0];
        }
        const averageDailySales = new ApexCharts(averageDailySalesEl, averageDailySalesConfig);
        averageDailySales.render();
    }

    // Earning Reports Bar Chart (Weekly Requests)
    // --------------------------------------------------------------------
    const weeklyEarningReportsEl = document.querySelector('#weeklyEarningReports');
    let weeklyEarningReportsConfig = {
        chart: {
            height: 161,
    ...
    if (typeof weeklyEarningReportsEl !== undefined && weeklyEarningReportsEl !== null) {
        // Try to get data from window object or use default
        if (typeof window.weeklyRequestsData !== 'undefined' && Array.isArray(window.weeklyRequestsData)) {
            weeklyEarningReportsConfig.series[0].data = window.weeklyRequestsData.map(item => item.count);
            weeklyEarningReportsConfig.xaxis.categories = window.weeklyRequestsData.map(item => item.day_ar || item.day);
        } else {
            // Default data
            weeklyEarningReportsConfig.series[0].data = [0, 0, 0, 0, 0, 0, 0];
            weeklyEarningReportsConfig.xaxis.categories = ['أحد', 'إثنين', 'ثلاثاء', 'أربعاء', 'خميس', 'جمعة', 'سبت'];
        }
    ...
    if (typeof supportTrackerEl !== undefined && supportTrackerEl !== null) {
        ...
        if (typeof window.totalRequests !== 'undefined' && typeof window.completedRequests !== 'undefined') {
            const completionPercentage = window.totalRequests > 0 
                ? Math.round((window.completedRequests / window.totalRequests) * 100) 
                : 0;
            supportTrackerOptions.series = [completionPercentage];
            supportTrackerOptions.labels = [window.completedRequests + ' ' + (window.completedRequestsLabel || 'مكتملة')];
        }
