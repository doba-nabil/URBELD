...
@section('title', __('website.edit_service'))

@section('profile-content')
    <div class="about-me-section">
        <div class="container">
            <h2 class="about-me-title mb-4">{{ __('website.edit_service') }}: {{ $service->getTranslation('title', app()->getLocale(), false) ?? $service->getTranslation('title', 'ar') }}</h2>
                    ...
                    <form action="{{ route('provider.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="title[en]" class="form-control" value="{{ old('title.en', $service->getTranslation('title', 'en', false) ?? '') }}">
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="sub_category_id" id="sub_category_id" class="form-select border-primary" required>
                                    ...
                                    @foreach($subCategories as $cat)
                                        <option value="{{ $cat->id }}" data-parent="{{ $cat->parent_id }}" {{ old('sub_category_id', $service->sub_category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea name="content[ar]" class="form-control" rows="5" required>{{ old('content.ar', $service->getTranslation('content', 'ar')) }}</textarea>
                            </div>

                            <div class="col-md-12 mb-3">
                                <label class="form-label">{{ __('website.service_content_en') }}</label>
                                <textarea name="content[en]" class="form-control" rows="4">{{ old('content.en', $service->getTranslation('content', 'en', false) ?? '') }}</textarea>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                @if($service->getFirstMediaUrl('services'))
                                    <div class="mt-3 bg-light p-2 rounded text-center">
                                        <p class="mb-2 text-muted fw-bold small">{{ __('website.current_image') }}:</p>
                                        <img src="{{ $service->getFirstMediaUrl('services') }}" height="80" class="rounded border shadow-sm">
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="mt-4 pt-3 border-top">
                            <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save"></i> {{ __('website.update_service') }}</button>
