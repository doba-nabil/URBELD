...
@section('dashboard-main')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-4">
            <h5 class="card-header border-bottom mb-3">{{ __('admin.edit') }}</h5>
            <div class="card-body">
                <form action="{{ route('supplier-offers.update', $offer->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" {{ old('user_id', $offer->user_id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label" for="title">اسم العرض <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-12 mb-3">
                            <label class="form-label" for="description">وصف العرض</label>
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label" for="discount_percentage">نسبة الخصم (%)</label>
                            <input type="number" step="0.01" max="100" class="form-control @error('discount_percentage') is-invalid @enderror" id="discount_percentage" name="discount_percentage" value="{{ old('discount_percentage', $offer->discount_percentage) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <a href="{{ route('supplier-offers.index') }}" class="btn btn-label-secondary">{{ __('admin.cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
