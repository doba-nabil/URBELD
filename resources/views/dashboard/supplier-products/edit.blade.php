                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" {{ old('user_id', $product->user_id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            @if($product->getFirstMediaUrl('product_images'))
                                <div class="mb-2">
                                    <img src="{{ $product->getFirstMediaUrl('product_images') }}" width="150" class="rounded border">
                                </div>
                            @endif
