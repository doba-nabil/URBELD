...
class SupplierOfferDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('image', function ($q) {
                if ($q->hasMedia('offer_images')) {
                    return '<img src="' . $q->getFirstMediaUrl('offer_images') . '" width="50" height="50" class="rounded">';
                }
            ...
            ->addColumn('discount_percentage', function ($q) {
                return '<span class="badge bg-label-success">' . ($q->discount_percentage ? e($q->discount_percentage) . '%' : '-') . '</span>';
            })
            ->addColumn('action', function ($q) {
                $editUrl = route('supplier-offers.edit', $q->id);
                $deleteUrl = route('supplier-offers.destroy', $q->id);
        ...
        <div class="dropdown">
            ...
            ->rawColumns(['action', 'image', 'title', 'supplier', 'discount_percentage'])
            ->setRowId('id');
    }
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('discount_percentage')->title('نسبة الخصم (%)'),
